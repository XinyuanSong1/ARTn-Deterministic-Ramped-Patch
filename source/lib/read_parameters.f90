...
!SECTION_____________________________ ART
 ...
 ! Use check connectivity to validate the reversibility of events
  ...
  call getenv ('LOCAL_LANCZOS',temporary)
  if (temporary == '') then
     LOCAL_LANCZOS = .false.
  if (LOCAL_FORCE) LOCAL_LANCZOS = .true.
  else
     read(temporary,*) LOCAL_LANCZOS
  ...
  !!__________________
  ! Read type of events
  ! Activation: global, local, list_local, list, local_coord and deterministic
...
if ( (TYPE_EVENTS .ne. 'global') .and. (TYPE_EVENTS .ne. 'local') .and. &
     ...
     (TYPE_EVENTS .ne. 'local_coord') .and. (TYPE_EVENTS .ne. 'deterministic') ) then
   write(*,*) 'Error : only global, local, list, local_coord, or deterministic type of events are accepted - provided: ',&
   & TYPE_EVENTS
   stop
end if
  ...
  if (TYPE_EVENTS .eq. 'local' .or. TYPE_EVENTS .eq. 'deterministic') then   !new
...
!SECTION_____________________________ HARMONIC WELL
  ...
  ! New parameters for threshold control
  EIGEN_THRESH_INTERVAL = -0.1d0					! new
  call getenv('EIGEN_THRESH_INTERVAL', temporary)			! new
  if (temporary .ne. '') read(temporary,*) EIGEN_THRESH_INTERVAL	! new

  EIGEN_THRESH_MIN = -1.0d0						! new
  call getenv('EIGEN_THRESH_MIN', temporary)				! new
  if (temporary .ne. '') read(temporary,*) EIGEN_THRESH_MIN		! new
...
!SECTION____________________________ CONVERGENCE
  ...
  if (temporary .eq. '') then
     PUSH_OVER_RELATIVE_ERROR = 1d+10
  else
...
subroutine write_parameters( )
  ...
  if (TYPE_EVENTS .eq. 'deterministic') then
     if (preferred_atom .gt. 0) then
        write(flog,'(1X,A39,i12)')   ' - Central atom for events           : ', preferred_atom
     else
        write(flog,'(1X,A39,A12)')   ' - Central atom for events           :   none (all equal) '
