...
module saddles
  ...
  real(kind=8) :: EIGEN_THRESH_INTERVAL		! new
  real(kind=8) :: EIGEN_THRESH_MIN		! new
...
subroutine find_saddle( success, saddle_energy )
  ...
  ! Save the original value			! new
  real(kind=8) :: original_thresh		! new
  original_thresh = EIGEN_THRESH		! new
...
!#endif
  ...
  !________
     ...
     if ( eventtype == "GUESS_DIRECTION" ) then
        ...
        selectcase( type_events )
          ...
          case( 'deterministic' )            !new 
              call deterministic_move( )     !new
        end select
     end if
  ...
  ! write inital eigenvalue threshold   # new
  write(*,*) 'EIGEN_THRESH adjusted to:', EIGEN_THRESH
  open(unit=FLOG, file=LOGFILE, status='unknown', action='write', position='append', iostat=ierror)
  write(FLOG,'(A,F10.4)') 'EIGEN_THRESH adjusted to: ', EIGEN_THRESH
  close(FLOG)
...
! Gradient EIGEN_THRESH ! new
  if (ret >= 500000 .and. ret <= 899999) then      !new
     do while (EIGEN_THRESH > EIGEN_THRESH_MIN)
        EIGEN_THRESH = EIGEN_THRESH + EIGEN_THRESH_INTERVAL
        ...
        if (ret < 500000 .or. ret > 899999) exit
     end do
     ! Restore the original threshold
     EIGEN_THRESH = original_thresh
  endif
...
!> ART deterministic search 
! In this method, only the selected atom (1 atom) will be moved initially
! New variables are defined:
!     setenv NDIRS --> how many directions in one sphere search.
!                      the higher the value, the more dense of the search  (default: 500)
!     setenv EXPLORE_RADIUS_INCREASE --> the the increased radius for each searching layer.
!     setenv MAX_RADIUS_LAYER --> the maximum number of searching layer

subroutine deterministic_move()
  ...
  ! Local variables
  integer :: i, ierror, that, ios, temp
  real(kind=8) :: ran3
  real(kind=8), dimension(3) :: boxl, invbox
  real(kind=8), parameter :: pi = 3.14159265358979323846d0
  real(kind=8), parameter :: phi = (1.d0 + sqrt(5.d0)) / 2.d0  ! Golden ratio
  real(kind=8) :: theta, r, zdir
  real(kind=8) :: xvec, yvec, zvec
  integer, save :: spiral_counter = 0
  integer, save :: layer_counter = 1
  integer, save :: NDIRS = 500
  integer, save :: MAX_RADIUS_LAYER = 3
  real(kind=8), save :: EXPLORE_RADIUS_INCREASE = 0.2d0
  logical, save :: env_initialized = .false.
  character(len=32) :: env_value
  logical :: overlap_found
  real(kind=8) :: dxij, dyij, dzij, dist2

  real(kind=8), dimension(:), pointer :: dx, dy, dz

  ! === Initialize environment-controlled inputs (once) ===
  if (.not. env_initialized) then
    call get_environment_variable("NDIRS", env_value, status=ios)
    if (ios == 0) then
      read(env_value, *, iostat=ios) temp
      if (ios == 0 .and. temp > 0) NDIRS = temp
    end if

    call get_environment_variable("EXPLORE_RADIUS_INCREASE", env_value, status=ios)
    if (ios == 0) then
      read(env_value, *, iostat=ios) EXPLORE_RADIUS_INCREASE
      if (ios /= 0) EXPLORE_RADIUS_INCREASE = 0.2d0
    end if

    call get_environment_variable("MAX_RADIUS_LAYER", env_value, status=ios)
    if (ios == 0) then
      read(env_value, *, iostat=ios) temp
      if (ios == 0 .and. temp > 0) MAX_RADIUS_LAYER = temp
    end if

    env_initialized = .true.
    write(*,*) 'INFO: NDIRS =', NDIRS
    write(*,*) 'INFO: EXPLORE_RADIUS_INCREASE =', EXPLORE_RADIUS_INCREASE
    write(*,*) 'INFO: MAX_RADIUS_LAYER =', MAX_RADIUS_LAYER
  end if
  ...
  if (preferred_atom < 0) then
    ...
    that = preferred_atom
    if (constr(that) .ne. 0) then
      write(*,*) ' ERROR: chosen atom is blocked in the geometry file'
      call end_art()
    end if
  end if

  if (that <= 0 .or. that > NATOMS) then
     write(*,*) 'ERROR: Invalid atom index: ', that
     call end_art()
  end if
  ...
  ! === Spiral sampling logic ===
  if (iproc == 0) then
    spiral_counter = spiral_counter + 1

    if (spiral_counter > NDIRS) then
      layer_counter = layer_counter + 1
      if (layer_counter > MAX_RADIUS_LAYER) then
        write(*,*) 'INFO: The deterministic search is done.'
        call end_art()
      end if

      INITSTEPSIZE = INITSTEPSIZE + EXPLORE_RADIUS_INCREASE
      spiral_counter = 1
      write(*,*) 'INFO: INITSTEPSIZE increased to', INITSTEPSIZE
      write(*,*) 'INFO: Restarting spiral sampling at layer ', layer_counter
    end if

    zdir  = 1.d0 - 2.d0 * spiral_counter / (NDIRS + 1)
    theta = 2.d0 * pi * spiral_counter / phi
    r     = sqrt(1.d0 - zdir*zdir)

    xvec = r * cos(theta)
    yvec = r * sin(theta)
    zvec = zdir

    dx(that) = INITSTEPSIZE * xvec
    dy(that) = INITSTEPSIZE * yvec
    dz(that) = INITSTEPSIZE * zvec

    ! === Overlap check before applying displacement ===
    overlap_found = .false.
    do i = 1, NATOMS
      if (i == that) cycle
      dxij = x(i) - (x(that) + dx(that))
      dyij = y(i) - (y(that) + dy(that))
      dzij = z(i) - (z(that) + dz(that))

      dxij = dxij - boxl(1) * nint(dxij * invbox(1))
      dyij = dyij - boxl(2) * nint(dyij * invbox(2))
      dzij = dzij - boxl(3) * nint(dzij * invbox(3))

      dist2 = dxij**2 + dyij**2 + dzij**2
      if (sqrt(dist2) < 0.8d0) then  ! if the distance between two atoms is smaller than 0.8 Ang,
        overlap_found = .true.
        exit
      end if
    end do
    ...
    !output information
    open(unit=99, file='dr_vector.log', status='unknown', action='write', position='append')
    open(unit=FLOG, file=LOGFILE, status='unknown', action='write', position='append', iostat=ierror)

    if (overlap_found) then
        write(*,*) 'INFO: Overlap detected. Skipping this deterministic search step.'
    
        ! Write to dr_vector.log
        write(99,'(A,I6,A,I3,A,F10.4,3F20.12,A)') 'Step: ', spiral_counter, ' Layer:', layer_counter, ' R =', INITSTEPSIZE, dx(that), dy(that), dz(that), '  ! overlapped'
    
        ! Write also to LOGFILE
        write(FLOG,'(A,I6,A,I3,A,F10.4,3F20.12,A)') 'Deterministic move - Step: ', spiral_counter, ' Layer:', layer_counter, ' R =', INITSTEPSIZE, dx(that), dy(that), dz(that), '  ! overlapped'
        ...
        ! Write to dr_vector.log
        write(99,'(A,I6,A,I3,A,F10.4,3F20.12)') 'Step: ', spiral_counter, ' Layer:', layer_counter, ' R =', INITSTEPSIZE, dx(that), dy(that), dz(that)
    
        ! Write also to LOGFILE
        write(FLOG,'(A,I6,A,I3,A,F10.4,3F20.12)') 'Deterministic move - Step: ', spiral_counter, ' Layer:', layer_counter, ' R =', INITSTEPSIZE, dx(that), dy(that), dz(that)
    ...
    ! === Normalize direction ===
    initial_direction = dr
    r = sqrt(dot_product(initial_direction, initial_direction))
    if (r > 0.d0) initial_direction = initial_direction / r
  end if

  ! === Update forces if needed ===
  if (LOCAL_FORCE) then
    use_local_forces = .true.
    call generate_local_region(that, pos)
  end if
...
end subroutine deterministic_move